    ...
    useEffect(() => {
        axios.get(`${urlToys}/${id}`)
            .then((answer: AxiosResponse<toyDTO>) => {
                answer.data.releaseDate = new Date(answer.data.releaseDate);
    ...
    function transformCoordinates(): coordinateDTO[]{
        if (toy?.branches){
            const coordinates = toy.branches.map(branch => {
                return {lat: branch.latitude, 
                    lng: branch.longitude, name: branch.name} as coordinateDTO;
            });
            return coordinates;
        }
    ...
    return (
        toy ?
            <div style={{ display: 'flex' }}>
                <div>
                    <h2>{toy.name} ({toy.releaseDate.getFullYear()})</h2>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                {toy.brands?.map(brand =>
                                    <div key={brand.id} style={{ marginBottom: '2px' }}>
                                        <img alt="Imagen Marca" src={brand.image}
